# Adapted from VideoX-Fun's pipeline_wan2_2_fun_inpaint.py (Apache-2.0).
# The denoising loop is extended with LanPaint flow-matching inner steps.
import inspect
import math
...
from .wan22_runtime import lanpaint_flow_step, latent_regenerate_mask
...
class Wan22LanPaintPipeline(DiffusionPipeline):
    r"""
    ...
    def __call__(
        ...
        shift: int = 5,
        lanpaint_steps: int = 1,
        lanpaint_cfg: float = 10000.0,
        lanpaint_lambda: float = 5.0,
        lanpaint_step_size: float = 0.15,
        ...
        # Keep both conditional and unconditional branches for cfg=0 ablations
        # and for LanPaint's second (BIG) guidance prediction.
        do_classifier_free_guidance = guidance_scale != 1.0 or lanpaint_steps > 0
        ...
        if init_video is not None:
            _, source_video_latents = self.prepare_mask_latents(
                None,
                init_video,
                batch_size,
                height,
        ...
        if init_video is None:
            raise ValueError("Standalone video inpainting requires --video and --mask")
        if not isinstance(self.scheduler, FlowMatchEulerDiscreteScheduler):
            raise ValueError("The standalone LanPaint baseline currently requires the Flow scheduler")
        regenerate_mask = latent_regenerate_mask(mask_video, latents)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if init_video is not None:
                    y_single = torch.cat([mask_latents, masked_video_latents], dim=1).to(device, weight_dtype)
                ...
                def predict_velocity(sample, cfg_scale):
                    model_input = torch.cat([sample] * 2) if do_classifier_free_guidance else sample
                    if hasattr(self.scheduler, "scale_model_input"):
                        model_input = self.scheduler.scale_model_input(model_input, t)
                    model_timestep = timestep_single.expand(model_input.shape[0], timestep_single.shape[-1])
                    model_y = torch.cat([y_single] * 2) if do_classifier_free_guidance else y_single
                    with torch.amp.autocast("cuda", dtype=weight_dtype), torch.cuda.device(device=device):
                        prediction = local_transformer(
                            x=model_input,
                    ...
                    if do_classifier_free_guidance:
                        ...
                        prediction = uncond + cfg_scale * (text - uncond)
                    return prediction

                run_lanpaint = lanpaint_steps > 0 and i < len(timesteps) - lanpaint_early_stop
                if run_lanpaint:
                    flow_sigma = self.scheduler.sigmas[i].to(device=device, dtype=latents.dtype)

                    def predict_x0(sample, _sigma):
                        normal_velocity = predict_velocity(sample, guidance_scale)
                        big_velocity = predict_velocity(sample, lanpaint_cfg)
                        sigma_view = flow_sigma.view(1, 1, 1, 1, 1)
                        return sample - sigma_view * normal_velocity, sample - sigma_view * big_velocity

                    latents, noise_pred = lanpaint_flow_step(
                        latents=latents,
                        source_latents=source_video_latents,
                    ...
                    noise_pred = predict_velocity(latents, guidance_scale)
